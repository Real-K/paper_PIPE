# -*- coding: utf-8 -*-
"""WP12c — 동상태 비교의 균형 진단 강화 (리뷰3 §7 대응).

리뷰 지적: distressed 층의 max |SMD| = 0.12 는 기계적 합격선을 넘진 않지만 **논문에서 가장 중요한 비교**이므로
(i) 어느 공변량이 0.12 인지 공개하고 (ii) 산업을 가중에 넣은 강건성과 (iii) 가중치 분포·극단가중·ESS 를 함께 내야 한다.
이 스크립트는 wp12b 와 **동일 설계·동일 시드**로 돌면서 그 세 가지를 추가 산출한다(추정치는 wp12b 와 같아야 한다).
"""
...
IND=nps.groupby("bn10")["업종"].first().to_dict()
def manuf(b):
    try: return 1.0 if 10<=int(str(IND.get(b,"99"))[:2])<=34 else 0.0
...
def unit(b,e,yr):
    ...
    return dict(bn=b,e=e,yr=yr,manuf=manuf(b),D=float(np.nanmean(v)-np.nanmean(bc)),logsize=float(np.nanmean(bc)),
...
def weights(A,Bd,cols):
    ...
    weights.last_converged=converged; weights.last_cols=list(X.columns)
    return w,Xs,ps
def smd(xt,xc,w):
...
def compare(A,Bd,tag,cols):
    ...
    r["ess"]=round(float(1/np.sum(w**2)),1); r["ps_converged"]=bool(getattr(weights,"last_converged",True))
    cn=getattr(weights,"last_cols",[f"x{j}" for j in range(Xs.shape[1])])
    per={cn[j]:round(float(smd(Xs[:len(A),j],Xs[len(A):,j],w)),3) for j in range(Xs.shape[1])}
    r["smd_by_covariate"]=per
    r["max_abs_smd"]=round(max(abs(v) for v in per.values()),3)
    r["worst_covariate"]=max(per,key=lambda k:abs(per[k]))
    ws=np.sort(w)[::-1]
    r["weight_diagnostics"]=dict(n_ctrl=int(len(w)),ess=r["ess"],ess_share=round(float(1/np.sum(w**2)/len(w)),4),
        top1_share=round(float(ws[0]),5),top10_share=round(float(ws[:10].sum()),4),top1pct_share=round(float(ws[:max(1,len(w)//100)].sum()),4),
        max_over_mean=round(float(ws[0]*len(w)),1))
...
print("\n[D2] 산업 추가 가중 강건성 (리뷰3 §7)")
R["D2_distress_1_ind"]=compare(TR[TR.distress==1],DR[DR.distress==1],"distressed · 산업 포함 가중",RICH_IND)
R["D2_distress_0_ind"]=compare(TR[TR.distress==0],DR[DR.distress==0],"non-distressed · 산업 포함",RICH_IND)
print("\n[D] 교차층 (부록)")
...
json.dump({"id":"WP12c","title":"동상태 비교 균형 진단(리뷰3 §7)","runs":R,"verdict":verdict,
           ...
           "kill":"통합 p10 CI∋0 → tail 열위 철회"},open(f"{OUT}/wp12c_balance.json","w"),ensure_ascii=False,indent=1)
print("\n"+verdict)
